...
fn main() {
    ...
    let total = 1000000;
    let (curr, next) = get_cups(input, total);
    let curr = play(curr, &next, 10000000);
    println!("{}", curr.count());
    let n1 = next[1].next().unwrap();
...
fn get_cups(input: &str, total: usize) -> (Rc<CupList>, Vec<Rc<CupList>>) {
    ...
    let mut next = vec![Rc::new(CupList::new_nil()); total + 1];
...
fn play(mut current: Rc<CupList>, next: &Vec<Rc<CupList>>, moves: u32) -> Rc<CupList> {
    for _ in 0..moves {
        //1.
